...
const MessagePage: React.FC = () => {
  ...
  const onFinish = () => {
    messageApi.batchSend(progress).then(() => {
      // Add type annotation to specify the type of 'res'
  ...
  return (
    <ProCard style={{ height: '100%' }}>
      ...
      >
        {records.length != 0 ? (
          records.map((item: StuCard) => (
            <Card
              key={item.smsRecordId}
              title={`${item.realName}    ${item.schoolNumber}`}
              style={{ width: 300, maxHeight: '46%', margin: '0px 10px' }}
            >
              <p>{item.smsContent}</p>
              <Space size={[0, 8]} wrap>
                {item.status === 1 ? (
